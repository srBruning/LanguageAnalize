...
public class SyntacticAnalyzerTest {
	...
	@Test
	public void expression1Test() throws Exception {
		assertNotNull(	syntactic.analyzer(newInputSyntactic("b/6")  )  );	
	}
	
	@Test
	public void expression2Test() throws Exception {
		assertNull( syntactic.analyzer(newInputSyntactic("a b")  )  );	
		assertEquals(syntactic.getErro().size(), 1);
		assertEquals(syntactic.getErro().get(0).getMsg(), "Esperava tokem de final de comando");
	}
	@Test
	public void expressionTest() throws Exception {
		assertNull( syntactic.analyzer(newInputSyntactic("a-/ b")  )  );
		assertEquals(syntactic.getErro().size(), 1);
		assertEquals(syntactic.getErro().get(0).getMsg(), "Expressão invalida!");	
	}
//
//
//	@Test
